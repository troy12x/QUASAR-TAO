...
def build_full_context(
    ...
    if byoc_mode and byoc_file_path:
        ...
        if byoc_path.exists():
            context_parts.append("# ════════════════════════════════════════════════════════════════════════")
            context_parts.append("# ⚠️  CRITICAL: EXPERT OPTIMIZED CODE (Bring-Your-Own-Code Mode)")
            context_parts.append("# ════════════════════════════════════════════════════════════════════════")
            context_parts.append("")
            context_parts.append("# 🔴 YOU MUST FOLLOW THESE INSTRUCTIONS EXACTLY:")
            context_parts.append("#")
            context_parts.append("# STEP 1: READ the expert code below CAREFULLY")
            context_parts.append("# STEP 2: UNDERSTAND the implementation approach and patterns used")
            context_parts.append("# STEP 3: USE the expert code as your PRIMARY REFERENCE")
            context_parts.append("# STEP 4: ADAPT the expert code to match the target file structure")
            context_parts.append("# STEP 5: VERIFY the function signature matches exactly")
            context_parts.append("# STEP 6: ENSURE all exports are correct")
            context_parts.append("#")
            context_parts.append("# ⚠️  DO NOT ignore or deviate from the expert code approach.")
            context_parts.append("# ⚠️  DO NOT create your own implementation - USE the expert's code.")
            context_parts.append("# ⚠️  DO NOT change the core logic - only adapt structure/imports if needed.")
            context_parts.append("")
            context_parts.append("# ────────────────────────────────────────────────────────────────────────")
            context_parts.append("# EXPERT CODE (USE THIS AS YOUR REFERENCE):")
            context_parts.append("# ────────────────────────────────────────────────────────────────────────")
            context_parts.append(format_file_content(byoc_path, repo_path))
            context_parts.append("")
            context_parts.append("# ────────────────────────────────────────────────────────────────────────")
            context_parts.append("# VERIFICATION CHECKLIST (MUST COMPLETE BEFORE OUTPUTTING):")
            context_parts.append("# ────────────────────────────────────────────────────────────────────────")
            context_parts.append("# [ ] Function signature matches expert code")
            context_parts.append("# [ ] Core implementation logic matches expert code")
            context_parts.append("# [ ] All required functions are exported correctly")
            context_parts.append("# [ ] Imports are compatible with repository structure")
            context_parts.append("# [ ] Code follows the patterns shown in expert code")
            context_parts.append("")
    ...
    # Strict instructions
    context_parts.append("# ════════════════════════════════════════════════════════════════════════")
    context_parts.append("# 🔴 MANDATORY INSTRUCTIONS - FOLLOW EXACTLY")
    context_parts.append("# ════════════════════════════════════════════════════════════════════════")
    context_parts.append("")
    context_parts.append("You are editing ONLY the target file. You MUST follow these instructions:")
    context_parts.append("")
    context_parts.append("1. FUNCTION EXPORT:")
    context_parts.append("   → Export `chunk_quasar` function EXACTLY as expected by the codebase")
    context_parts.append("   → Check __init__.py to see how it's exported")
    context_parts.append("   → Match the function signature from relevant source files")
    context_parts.append("")
    context_parts.append("2. PUBLIC API:")
    context_parts.append("   → DO NOT change function names, parameters, or return types")
    context_parts.append("   → DO NOT remove or rename exported functions")
    context_parts.append("   → Maintain backward compatibility")
    context_parts.append("")
    context_parts.append("3. IMPORTS AND SIGNATURES:")
    context_parts.append("   → Preserve ALL existing imports unless explicitly improving performance")
    context_parts.append("   → Match function signatures from the codebase")
    context_parts.append("   → Use imports that exist in the repository")
    context_parts.append("")
    context_parts.append("4. COMPATIBILITY:")
    context_parts.append("   → Ensure your code works with files that import the target file")
    context_parts.append("   → Check how other files use the functions you're implementing")
    context_parts.append("   → Test compatibility by reviewing import statements in source files")
    context_parts.append("")
    context_parts.append("5. CODE STYLE:")
    context_parts.append("   → Follow the code style and patterns used in the repository")
    context_parts.append("   → Match indentation, naming conventions, and structure")
    context_parts.append("   → Use similar error handling and logging patterns")
    context_parts.append("")
    if byoc_mode and byoc_file_path:
        context_parts.append("6. EXPERT CODE PRIORITY:")
        context_parts.append("   → If expert code was provided, USE IT as your primary reference")
        context_parts.append("   → Adapt expert code structure to match repository conventions")
        context_parts.append("   → DO NOT ignore expert code - it contains the intended approach")
        context_parts.append("")
    context_parts.append("")
