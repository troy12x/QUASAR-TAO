...
def build_full_context(
    ...
    if byoc_mode and byoc_file_path:
        ...
        if byoc_path.exists():
            context_parts.append("# ════════════════════════════════════════════════════════════════════════")
            context_parts.append("# EXPERT OPTIMIZED CODE (Bring-Your-Own-Code Mode)")
            context_parts.append("# ════════════════════════════════════════════════════════════════════════")
            context_parts.append(format_file_content(byoc_path, repo_path))
            context_parts.append("")
            context_parts.append("# INSTRUCTION: Review the expert-optimized code above.")
            context_parts.append("# Ensure it correctly exports the required function(s) and maintains the public API.")
            context_parts.append("# Output the final, verified version.")
            context_parts.append("")
    ...
    # Strict instructions
    context_parts.append("# ════════════════════════════════════════════════════════════════════════")
    context_parts.append("# STRICT INSTRUCTIONS")
    context_parts.append("# ════════════════════════════════════════════════════════════════════════")
    context_parts.append("")
    context_parts.append("You are editing ONLY the target file. You MUST:")
    context_parts.append("1. Export the function `chunk_quasar` exactly as expected by the rest of the codebase")
    context_parts.append("2. Do not change the public API")
    context_parts.append("3. Preserve all existing imports and function signatures unless explicitly improving performance")
    context_parts.append("4. Ensure compatibility with all files that import or use the target file")
    context_parts.append("5. Follow the code style and patterns used in the repository")
    context_parts.append("")
